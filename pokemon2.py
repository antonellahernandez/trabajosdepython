...
posicion = lista_entrenadores.search("Ash Ketchum", "nombre")
...
if mayor_entrenador is not None:
    ...
    if mayor_pokemon is not None:
        ...
        print(
            f"El entrenador {mayor_entrenador.nombre} no tiene Pokémon en su lista.")
else:
...
for pos in range(lista_entrenadores.size()):
    ...
    for pos_pokemon in range(sublista.size()):
        pokemon = sublista.get_element_by_index(pos_pokemon)
        if pokemon.tipo.lower() in tipos_a_verificar or (pokemon.subtipo and pokemon.subtipo.lower() in tipos_a_verificar):
            tiene_tipos = True
            break
    if tiene_tipos:
...
for pos_entrenador in range(lista_entrenadores.size()):
    entrenador, sublista = lista_entrenadores.get_element_by_index(
        pos_entrenador)
    for pos_pokemon in range(sublista.size()):
        pokemon = sublista.get_element_by_index(pos_pokemon)
...
print(f"{cantidad_entrenadores_con_pokemon} entrenadores tienen a {nombre_pokemon} en sus listas.")
pokemon_repetidos = {}
for pos_entrenador in range(lista_entrenadores.size()):
    entrenador, sublista = lista_entrenadores.get_element_by_index(
        pos_entrenador)
...
if pos_entrenador is not None:
    ...
    if pos_pokemon is not None:
        ...
        print(
            f"Entrenador {entrenador.nombre} tiene al Pokémon {pokemon.nombre}:")
